import torch
from torch.utils.data.dataloader import DataLoader  # need torch.__version__ == '1.1.0'
...
class tools():
    ...
    class occ_check_model():
        ...
        def __init__(self, occ_type='for_back_check', occ_alpha_1=1.0, occ_alpha_2=0.05, sum_abs_or_squar=True, obj_out_all='all'):
            ...
            :param occ_alpha_1: threshold            :param occ_alpha_2:
    ...
    def torch_warp_mask(cls, x, flo):
        ...
        if x.is_cuda:
            grid = grid.cuda()
        # print(grid.shape,flo_flct.shape,'...')
        vgrid = grid + flo
    ...
    def write_flow(cls, flow, filename):
        """
        write optical flow in Middlebury .flo_flct format
        :param flow: optical flow map
    ...
    def read_flow(cls, filename):
        """
        read optical flow from Middlebury .flo_flct file
        ...
        if 202021.25 != magic:
            print('Magic number incorrect. Invalid .flo_flct file')
        else:
            w = np.fromfile(f, np.int32, count=1)
            h = np.fromfile(f, np.int32, count=1)
            # print("Reading %d x %d flo_flct file" % (h, w))
