...
from core.utils.augmentor import FlowAugmentor, SparseFlowAugmentor, FlowAugmentorMSY, FlowAugmentorCL
    ...
    def __init__(self, aug_params=None, sparse=False):
        ...
        if aug_params is not None:
            if sparse:
                ...
                self.augmentor = FlowAugmentorCL(**aug_params)
...
class Solar(FlowDataset):
    ...
    def __init__(self, aug_params=None, split='training',
                 root='F:/User_Folders/20222104112CL/PWCNet-MSY/datasets/Solar0513', dstype='img'):
        super(Solar, self).__init__(aug_params)
        flow_root = glob(osp.join(root, split, 'flow', '*.flo_flct'))
...
class Solar_true(FlowDataset):
    """尚老师建议的真实前后两帧版"""

    def __init__(self, aug_params=None, split='training',
                 root='F:/User_Folders/20222104112CL/PWCNet-MSY/datasets/Solar0513', dstype='img'):
        super(Solar_true, self).__init__(aug_params)
        flow_root = glob(osp.join(root, split, 'flow', '*.flo_flct'))
        image1_root = glob(osp.join(root, split, 'frame1', '*.jpg')) + \
                      glob(osp.join(root, split, 'frame1', '*.png'))
        image2_root = glob(osp.join(root, split, 'frame2', '*.jpg')) + \
                      glob(osp.join(root, split, 'frame2', '*.png'))
...
class SolarDis(FlowDataset):
    def __init__(self, aug_params=None, split='train', root='datasets/Solar0513_displacement', dstype1='img1',
                 dstype2='img2', istest='train'):
        super(SolarDis, self).__init__(aug_params)
        flow_root = glob(osp.join(root, split, 'flo_flct', '*.flo_flct'))
...
class SolarNo(FlowDataset):
    def __init__(self, aug_params=None, split='train', root='datasets/Solar0513_nopadding', dstype1='img1',
                 dstype2='img2', istest='train'):
        super(SolarNo, self).__init__(aug_params)
        flow_root = glob(osp.join(root, split, 'flo_flct', '*.flo_flct'))
        image1_root = glob(osp.join(root, split, dstype1, '*.jpg')) + \
                      glob(osp.join(root, split, dstype1, '*.png'))
        image2_root = glob(osp.join(root, split, dstype2, '*.jpg')) + \
...
class SolarZero(FlowDataset):
    def __init__(self, aug_params=None, split='train',
                 root='F:/User_Folders/20222104112CL/PWCNet-MSY/datasets/Solar0513_nppad', dstype1='img1',
                 dstype2='img2',
                 istest='train'):
...
class Optical(FlowDataset):
    def __init__(self, aug_params=None, split='train', root='F:/User_Folders/20222104112CL/PWCNet-MSY/datasets/Optical',
                 dstype1='img1', dstype2='img2',
                 istest='train'):
...
class Optical_true(FlowDataset):
    def __init__(self, aug_params=None, split='train', root='F:/User_Folders/20222104112CL/PWCNet-MSY/datasets/Optical',
                 dstype1='frame1', dstype2='frame2',
                 istest='train'):
        super(Optical_true, self).__init__(aug_params)
        flow_root = glob(osp.join(root, split, 'flow', '*.flo_flct'))
        image1_root = glob(osp.join(root, split, dstype1, '*.jpg')) + \
                      glob(osp.join(root, split, dstype1, '*.png'))
        image2_root = glob(osp.join(root, split, dstype2, '*.jpg')) + \
...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='datasets/Sintel', dstype='clean'):
        ...
        for scene in os.listdir(image_root):
            ...
            if split != 'test':
                self.flow_list += sorted(glob(osp.join(flow_root, scene, '*.flo_flct')))
...
class FlyingChairs(FlowDataset):
    def __init__(self, aug_params=None, split='train', root='datasets/FlyingChairs_release/data'):
        ...
        flows = sorted(glob(osp.join(root, '*.flo_flct')))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        # sintel_clean = MpiSintel(aug_params, split='training', dstype='clean')
        # sintel_final = MpiSintel(aug_params, split='training', dstype='final')
        solar = Solar_true(aug_params, split='training', dstype='img')
        # solardis = SolarDis(aug_params, split='train')
        train_dataset = 200 * solar
    elif args.stage == 'solardis':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        solardis = SolarDis(aug_params, split='train')
        train_dataset = 50 * solardis
        ...
        # aug_params=None, split='train', root='datasets/Solar0513_displacement',
        # dstype1='img1', dstype2='img2', istest='train'
    # print(os.path)
