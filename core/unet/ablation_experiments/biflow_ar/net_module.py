...
from core.unet.ablation_experiments.biflow_ar.res_bottle import ResidualBlock, Block
...
class FRFlowDecoder(nn.Module):
    """
    流解码器模块，输出各层的估计光流，输出的tensor张量尺寸为N*2*H*W，其中H和W根据每层分辨率变化
    SEBlock：是一种通道权重选择方案，可以换取更好的方法进行调整改进
    对于最终的输出，建议加一层精细化流操作，实现迭代优化的效果，应该能取得更优结果，类似原PWCNet模型的代码的迭代细化操作
    """
    ...
    def forward(self, f1_pyramid, f2_pyramid, cnet_pyramid):
        ...
        # output_3 = self.conv_output[3](torch.cat([multi1_3, multi2_3], dim=1))
        ...
        # output_2 = self.conv_output[2](torch.cat([multi1_2, multi2_2], dim=1)) + init_flow2
        ...
        # output_1 = self.conv_output[1](torch.cat([multi1_1, multi2_1], dim=1)) + init_flow1
        ...
        # output_0 = self.conv_output[0](torch.cat([multi1_0, multi2_0], dim=1)) + init_flow0
...
# def correlate(input1, input2, p_size):
