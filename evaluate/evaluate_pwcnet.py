...
from core.pwc_net.PWCNet import PWCDCNet
...
from utils.evaluationIndicators.utils import ssim, rv_rm
...
def create_sintel_submission(model, iters=32, warm_start=False, output_path='sintel_submission'):
    ...
    for dstype in ['clean', 'final']:
        ...
        for test_id in range(len(test_dataset)):
            ...
            output_file = os.path.join(output_dir, 'frame%04d.flo_flct' % (frame + 1))
...
def validate_solar(model, iters=2):  # 32
    ...
    for val_id in range(len(val_dataset)):
        ...
        intWidth = image1.shape[-1]
        intHeight = image1.shape[-2]

        intPreprocessedWidth = int(math.floor(math.ceil(intWidth / 64.0) * 64.0))
        intPreprocessedHeight = int(math.floor(math.ceil(intHeight / 64.0) * 64.0))

        image1 = F.interpolate(image1, (intPreprocessedHeight, intPreprocessedWidth), mode='bilinear')
        image2 = F.interpolate(image2, (intPreprocessedHeight, intPreprocessedWidth), mode='bilinear')

        flow_pr = model(image1, image2)
        flow = F.interpolate(flow_pr[0] * 20, (intPreprocessedHeight, intPreprocessedWidth), mode='bilinear')
        image1 = F.interpolate(image1, (intHeight, intWidth), mode='bilinear')
        image2 = F.interpolate(image2, (intHeight, intWidth), mode='bilinear')
        flow = F.interpolate(flow, (intHeight, intWidth), mode='bilinear')
    ...
    total_rm /= len(val_dataset)
    total_rv /= len(val_dataset)
...
def validate_solardis(model, iters=2):  # 32
    ...
    for split in ['train', 'test']:
        ...
        for val_id in range(len(val_dataset)):
            ...
            flow_pr = model(image1, image2)
            flow = F.interpolate(flow_pr[0] * 20, (intPreprocessedHeight, intPreprocessedWidth), mode='bilinear').cpu()
...
if __name__ == '__main__':
    ...
    # run code : --model=models/pwcnet-solar.pth --dataset=solardis
    ...
    model = torch.nn.DataParallel(PWCDCNet())
