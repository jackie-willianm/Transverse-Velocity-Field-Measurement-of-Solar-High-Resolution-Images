...
import datasets
from core.raftnet.ablation_experiment.baseline.utils import frame_utils

from core.dipnets.dip import DIP
from core.raftnet import InputPadder, forward_interpolate
...
@torch.no_grad()
def validate_solar(model, iters=20):  # 32
...
@torch.no_grad()
def validate_solarzero(model, iters=20):  # 32
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    dstype1 = 'img1'
...
@torch.no_grad()
def validate_optical(model, iters=20):  # 32
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    dstype1 = 'img1'
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(DIP(max_offset=192, mixed_precision=False, test_mode=True))
