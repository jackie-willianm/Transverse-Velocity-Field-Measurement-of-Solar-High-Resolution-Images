import sys
import math
from core.flownetc.original.flownet import FlowNetC
...
def validate_solar(model, iters=2):  # 32
    ...
    for val_id in range(len(val_dataset)):
        ...
        warp_img = flow_warp(image1, flow_pr[0])
        res_variance, res_mean = rv_rm(warp_img, image2, 10)
        res_ssim = ssim(warp_img, image2, 10)
        total_ssim += res_ssim
        total_rm += res_mean
        total_rv += res_variance
        _, h_gt, w_gt = flow_gt.size()
        flow_pre_full = F.interpolate(flow_pr[0] * 4, (h_gt, w_gt), mode='bilinear', align_corners=False)
        # flow = padder.unpad(flow_pre_full).cpu()
        flow = flow_pre_full.squeeze(0).cpu()
        flow[0, :, :] *= float(intWidth) / float(intPreprocessedWidth)
        flow[1, :, :] *= float(intHeight) / float(intPreprocessedHeight)
        crop_flow = flow[:, 10:-10, 10:-10] - flow_gt[:, 10:-10, 10:-10]

        epe = torch.sum(crop_flow ** 2, dim=0).sqrt()
...
def validate_sintel(model, iters=32):
    ...
    for dstype in ['clean', 'final']:
        ...
        for val_id in range(len(val_dataset)):
            ...
            flow_pr = model([image1, image2])
            _, h_gt, w_gt = flow_gt.size()
            flow_pre_full = F.interpolate(flow_pr[0], (h_gt, w_gt), mode='bilinear', align_corners=False)
            flow = flow_pre_full.squeeze(0).cpu()
...
if __name__ == '__main__':
    ...
    model = torch.nn.DataParallel(FlowNetC())
