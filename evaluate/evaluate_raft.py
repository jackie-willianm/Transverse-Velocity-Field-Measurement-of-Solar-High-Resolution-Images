...
from utils.evaluationIndicators.utils import rv_rm, ssim
...
from core.raftnet.utils import flow_viz
from core.raftnet.utils import frame_utils

from core.raftnet.raft import RAFT
from core.raftnet.utils.utils import InputPadder, forward_interpolate
...
def validate_solar(model, iters=2):  # 32
    ...
    a_ssim = total_ssim / len(val_dataset)
    a_rm = total_rm / len(val_dataset)
    a_rv = total_rv / len(val_dataset)
    ...
    print("Validation (%s) a_ssim: %f, a_rm: %f, a_rv: %f" % (dstype, a_ssim, a_rm, a_rv))
    return results
...
def validate_solarzero(model, iters=2):  # 32
    ...
    for split in ['test']:  # 'train', 'test'
        ...
        print("Validation (%s) a_ssim: %f, a_rm: %f, a_rv: %f" % (split, a_ssim, a_rm, a_rv))
