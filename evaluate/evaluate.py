...
# from core.unet.improve_res_net.module04_singleflow.raft import FRAFT
...
# from core.unet.improve_res_net.module04_biflow.raft import FRAFT
...
def validate_solar(model, iters=2):  # 32
    ...
    results = {}
    dstype = 'img'
    val_dataset = datasets.Solar_true(split='test', dstype=dstype)
    epe_list = []
    epenoc_list = []
    occu_mask_list = []
    flow_u = 0
...
def validate_solar_upflow(model, iters=2):  # 32
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    dstype = 'img'
    ...
    for val_id in range(len(val_dataset)):
        ...
        # flow_low, flow_pr, occu_mask = model(image1, image2, iters=iters, test_mode=True, add_bw=True)
        with torch.no_grad():
            input_dict = {'im1': image1, 'im2': image2, 'if_loss': False}
            output_dict = model(input_dict)
            flow_fw, flow_bw = output_dict['flow_f_out'], output_dict['flow_b_out']
            pred_flow = flow_bw
            # metric test
            warp_img = flow_warp(image1, pred_flow)
            res_variance, res_mean = rv_rm(warp_img, image2, 10)
            res_ssim = ssim(warp_img, image2, 10)
            total_ssim += res_ssim
            total_rm += res_mean
...
def validate_optical(model, iters=2):  # 32
    ...
    total_ssim, total_rm, total_rv = 0, 0, 0
    for split in ['test']:  # 'train', 'test'
        val_dataset = datasets.Optical_true(split=split, dstype1=dstype1, dstype2=dstype2, istest='test')
        flow_u = 0
        flow_v = 0
        epe_list = []
        for val_id in range(len(val_dataset)):
...
def validate_optical_upflow(model, iters=2):  # 32
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}
    dstype1 = 'frame1'
...
def eva(args):
    # run code : --model=models/raft-solar.pth --dataset=solardis
    model = torch.nn.DataParallel(FRAFT(args))  # unet
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        if args.dataset == 'chairs':
            ...
            validate_solar_upflow(model.module)
            validate_solarzero(model.module)
            validate_optical_upflow(model.module)
